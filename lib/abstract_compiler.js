...
class AbstractCompiler {
  ...
  buildImports() {
    ...
    for (var idx = 0; idx < imports.length; idx++) {
      ...
      source.replace(import_.range[0], import_.range[1], import_.loc, replacement);
    }
  }
  ...
  buildExports() {
    ...
    for (var export_ of exports_) {
      ...
      if (export_.default) {
        ...
        source.replace(export_.range[0], export_.range[1], export_.loc, replacement);
        ...
        if (export_.declaration.type === "VariableDeclaration") {
          ...
          source.replace(export_.range[0], export_.declaration.range[0] - 1, export_.loc, "");
          // add a new line
          replacement = this.doExportDeclaration(name);
          source.replace(export_.range[1], export_.range[1], export_.loc, replacement);
          ...
          replacement = this.doExportDeclaration(name);
          source.replace(export_.range[1] + 1, export_.range[1] + 1, export_.loc, replacement);
        } else if (export_.declaration.type === "Identifier") {
          name = export_.declaration.name;

          replacement = this.doExportDeclaration(name);
          source.replace(export_.range[0], export_.range[1] - 1, export_.loc, replacement);
        }
